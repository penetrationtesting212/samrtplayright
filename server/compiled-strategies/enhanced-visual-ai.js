...
class EnhancedVisualAIStrategy extends BaseHealingStrategy {
  model.LayersModel | null = null;
  isModelLoaded = false;
  featureExtractor.LayersModel | null = null;
  ...
  async createFeatureExtractor() {
    ...
    this.featureExtractor = tf.sequential({
      layers: [
        ...
        tf.layers.dense({ units, activation: 'relu' }),
        tf.layers.dense({ units }) // Feature vector output
      ]
    });
  ...
  async extractElementFeatures(context, screenshot) {
    try {
  ...
  async findAISimilarElements(targetFeatures.Tensor, context) {
    return await this.page.evaluate(
      ...
      { 
        contextData,
        targetFeaturesData targetFeatures.data()
      }
    );
  }

  async calculateAdvancedVisualScore(element, context) {
    return await this.page.evaluate(
      ({ el, ctx }) => {
        ...
        return weights > 0 ? score / weights ;
      },
      { el, ctx }
    );
  }

  calculateAdaptiveSizeSimilarity(size1: {width, height}, size2: {width, height}) {
  ...
  async fallbackToBasicVisual(context) {
...
module.exports = { EnhancedVisualAiStrategy };